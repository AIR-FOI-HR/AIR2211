...
public class DnevnaAktivnostService {
    ...
    public DnevnaAktivnost AddDnevnaAktivnost(DnevnaAktivnost dnevnaAktivnost) {
        return dnevnaAktivnostRepository.save(dnevnaAktivnost);
    }

    public Long DeleteDnevnaAktivnost(int id) {
        if (dnevnaAktivnostRepository.existsById(id)) {
            return dnevnaAktivnostRepository.deleteByIdAktivnost(id);
        } else {
            return 0L;
        }
    }
    ...
    public DnevnaAktivnost UpdateDnevnaAktivnost(DnevnaAktivnost updatedDnevnaAktivnost) {
        ...
        if (dnevnaAktivnostZaUpdate.isEmpty()) {
            return null;
        } else {
            return dnevnaAktivnostRepository.save(updatedDnevnaAktivnost);
        }
    }
}
